...
import './style.css'
import { FaHeartPulse, FaUserDoctor, FaCalendarDays, FaRobot, FaArrowDown} from "react-icons/fa6";
        ...
        <div className="section_container mb-20" ref={containerRef}>
            <section
                ...
                <main className={`flex w-full z-10
                    ...
                    >
                        <h2 className="text-7xl font-bold mb-6">Chào mừng bạn đến với bệnh viện Lục Lâm 2</h2>
                        <div className="intro">
                            <p className="text-2xl mb-4 font-bold">Chúng tôi cung cấp dịch vụ chăm sóc sức khỏe chất lượng cao.</p>
                            <p className="text-2xl mb-4 font-bold">Đội ngũ bác sĩ và nhân viên y tế chuyên nghiệp, tận tâm.</p>
                            <p className='text-2xl mb-4 font-bold'>
                              <span>Hỗ trợ tư vấn trực tuyến </span>
                              <span className='text-[#3D90D7]'>24/24 với AI</span>
                            </p>
                            <p className="text-2xl mb-4 font-bold">Liên hệ với chúng tôi để biết thêm thông tin.</p>
                        </div>
            ...
            >
              <div className={`top_features flex-1 mt-20 ${currentSection === 1 ? 'animate-fadeScaleIn' : 'animate-fadeScaleOut'} mb-10`}>
                <h2 className='text-4xl font-bold mb-6 text-center text-[#3D90D7]'>
                  Chăm sóc sức khoẻ cực đơn giản</h2>
              ...
              >
                 <h2 className='text-4xl font-bold mb-6 text-center text-[#3D90D7]'>
                  Các chuyên khoa tại bệnh viện</h2>
                <motion.div
                ...
                >
                      ...
                      {displayedItems.map((item, index) => {
                        ...
                        return (
                              <Link href={item.link || '#'} key={index}>
                                <ButtonCardComponent key={index} propStyle={{width: 300}}>
                                  <Icon size={80} color='#0065F8' />
                                  <p className='text-[#0065F8] font-bold'>{item.name}</p>
                                </ButtonCardComponent>
                              </Link>
                        )
                      })}
                  ...
                  <div className="showMore text-center mt-10 mb-10 items-center justify-center flex">
                      <div className='flex items-center justify-center gap-2 button_group w-50'
                        onClick={() => setShowAll(!showAll)} 
                      >
                        <p className='text-[#0065F8] font-bold cursor-pointer text-2xl'
                          >{showAll ? 'Ẩn bớt' : 'Xem thêm'}</p>
                        <FaArrowDown className={`arrow-icon text-[#0065F8] text-2xl cursor-pointer ${showAll ? 'arrow-up' : 'arrow-down'}`}
                          />
                      </div>
                  </div>     
                </motion.div>     
            ...
            >
                ...
                <TitleComponent title="Các trưởng khoa dày dặn kinh nghiệm" />
                      <div className="head_group grid grid-cols-2 md:grid-cols-4 gap-6">
                        <CardComponent imgUri='https://www.fvhospital.com/wp-content/uploads/2016/09/dr-le-dinh-phuong.jpg'
                      ...
                      <div className="top-news">
                        <div className="side-news">
                          {sideNews.map((news, index) => (
                            <NewsCardComponent articles={news} key={index}></NewsCardComponent>
                          ))}
                        </div>
                        <div className="main-news">
                          {mainNews.map((news, index) => (
                            <NewsCardComponent key={index} isMainNews articles={news}></NewsCardComponent>
                          ))}
                        </div>
                      </div>
                      <div className="bottom-news grid grid-cols-1 md:grid-cols-3 gap-6">
                        {otherNews.map((news, index) => (
                          <NewsCardComponent key={index} isOtherNews articles={news}></NewsCardComponent>
                        ))}
                      </div>
                  </div>
            </section>
